...
interface Voter {
  ...
  name: string
  username: string
  year_level?: YearGroup
  year_section?: SectionGroup
  has_voted?: boolean
}
...
export default function ReportsPrintVoters({ event, voters, filters, signatories }: Props) {
  const { system_settings } = usePage<SharedData>().props

  const titleSuffix = useMemo(() => {
    if (filters.status === 'voted') return 'VOTERS WHO VOTED'
    if (filters.status === 'not_voted') return 'VOTERS WHO HAVE NOT VOTED'
                  ...
                  <tbody>
                    <tr>
                      <td className="p-1 font-bold">
                        Total Records: {voters.length.toLocaleString()}
                      </td>
                      <td className="p-1">
                        {filters.search ? <span className="text-[11px]">Search: "{filters.search}"</span> : null}
                      </td>
                      <td className="p-1">
                        {filters.year_level_id ? <span className="text-[11px]">Year Level: {filters.year_level_id}</span> : null}
                      </td>
                      <td className="p-1">
                        {filters.year_section_id ? <span className="text-[11px]">Section: {filters.year_section_id}</span> : null}
                      </td>
                    </tr>
                  </tbody>
                </table>
                  ...
                  <table className="w-full border-collapse border border-black table-fixed -mt-[1px]">
                    <thead>
                      <tr className="bg-gray-100">
                        <th className="border border-black px-2 py-1 text-left w-48">Voter Name</th>
                        <th className="border border-black px-2 py-1 text-left w-32">Voter ID</th>
                    ...
                    <tbody>
                      {voters.map((v) => (
                        <tr key={v.id}>
                          <td className="border border-black px-2 py-1 uppercase">{v.name}</td>
                          <td className="border border-black px-2 py-1 font-mono">{v.username}</td>
                          <td className="border border-black px-2 py-1 uppercase">
                            {v.year_level?.name} {v.year_section?.name ? `- ${v.year_section?.name}` : ''}
                          </td>
                      ...
                      {voters.length === 0 && (
                        <tr>
                          <td colSpan={4} className="border border-black px-2 py-4 text-center italic">
